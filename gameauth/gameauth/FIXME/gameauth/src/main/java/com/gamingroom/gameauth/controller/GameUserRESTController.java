...
// Define a URL path annotation for gameusers
@Path("/gameusers/")
...
public class GameUserRESTController {
    ...
    // User must be authenticated. Allow API access to users with USER role
    @RolesAllowed({ "USER" })
    @GET
    @Path("/{id}")
    ...
    // User must be authenticated. Allow API access to users with ADMIN role
    @RolesAllowed({ "ADMIN" })
    @POST
